...
const dispatch = useDispatch();
  ...
  return (
    <div>
        <h1>Favorites List</h1><img src={star} alt='favorites'></img>
        <div data-aos="flip-up"
        className='container-cards'>
        {favList.length > 0 ? favList.map((item,i) =>{
          
          return <Link style={{ textDecoration: 'none' }} key={i}  to="/" state={{from: item.name}}> 
          ...
          <button onClick={(e) => remove(item.id,e)} className={ "btn btn-danger"}>{'Remove from Favorites'}<img src={star} alt='favorites'></img></button>
       </div>
      </div>
     </Link>
        }) : <h2 className='noFavTitle'>No favorites yet...add your own favorites!</h2>}
    </div>
